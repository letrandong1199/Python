...
def Read_Number(count,num,typ):
    if typ=='vie':
        ...
        if (count == 1):
            ...
            return Dezons_eng(num)
        elif count == 3:
            return Hunreds_eng(num)
...
digits_eng = ['billion', 'million', 'thousand', '.']
...
def Hunreds_eng(num):
    ...
    if (num%100!=0):
        result.append('and')
        result += Dezons_eng(num % 100)
    return result
def integer_to_english_numeral(num):
    divisor = 10 ** 9
    result = []
    i = 0
    while num != 0:
        if (num // divisor != 0):
            count = Local_length(num // divisor)
            result += Read_Number(count, num // divisor,'eng')
            result.append(digits_eng[i])
            if(num%1000==0)
            num %= divisor
            divisor //= 1000
            i += 1
    ...
    result_string=" ".join(i for i in result)
    return result_string
print(integer_to_english_numeral(12312312312))
